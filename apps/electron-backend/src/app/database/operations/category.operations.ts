        ...
        return [
            {
                ...
                type,
                xtreamId,
                // Parental control default: a category matching the adult
                // keyword list starts hidden the first time it's ever seen,
                // same as one explicitly listed in hiddenCategoryXtreamIds
                // (backup restore). Only applies on first import for this
                // playlist/type (saveCategories short-circuits otherwise),
                // so it never overrides a user's later choice to reveal it.
                hidden:
                    hiddenSet.has(xtreamId) ||
                    isAdultCategoryName(category.category_name),
            },
        ];
    });
}
