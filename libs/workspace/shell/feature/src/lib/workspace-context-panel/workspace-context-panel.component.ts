...
export class WorkspaceContextPanelComponent {
    ...
    openManageCategories(): void {
        ...
        if (this.parentalControl.isEnabled()) {
            void import('@iptvnator/portal/shared/ui').then(
                ({ ParentalPinDialogComponent }) => {
                    const pinDialogRef = this.dialog.open(
                        ParentalPinDialogComponent,
                        {
                            width: '360px',
                            data: {
                                messageKey:
                                    'PARENTAL_CONTROL.PIN_PROMPT_CATEGORY_MANAGEMENT',
                            },
                        }
                    );
                    pinDialogRef
