...
import { isAdultCategoryName } from '@iptvnator/shared/interfaces';
...
export class CategoryManagementDialogComponent implements OnInit {
    ...
    /** Whether any loaded category (selected or already hidden) looks like
     *  an adult category — controls whether the auto-hide button shows at
     *  all, since most playlists have none. */
    readonly hasAdultCategories = computed(() =>
        this.categories().some((c) => isAdultCategoryName(c.name))
    );
    ...
    /**
     * Deselects (marks to be hidden on Save) every category whose name
     * matches the adult keyword list — a manual, retroactive equivalent of
     * the always-on default applied when a playlist's categories are first
     * imported (which only covers categories not already in the database).
     * Already-hidden non-adult categories are left as the user set them.
     */
    autoHideAdultCategories(): void {
        this.categories.update((cats) =>
            cats.map((c) =>
                isAdultCategoryName(c.name) ? { ...c, selected: false } : c
            )
        );
    }
