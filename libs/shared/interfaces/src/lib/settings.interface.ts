...
export interface Settings {
    ...
    tmdb?: TmdbSettings;
    /**
     * PIN-gated hiding of adult Xtream/Stalker categories. Enabled by
     * default with a starter PIN (see DEFAULT_PARENTAL_CONTROL_SETTINGS) so
     * adult categories are hidden out of the box; the user is expected to
     * change the PIN on first use via the Parental Control settings section.
     */
    parentalControl?: ParentalControlSettings;
}

/**
 * Parental control: hides Xtream/Stalker categories whose name matches
 * `autoHideKeywords` (see `isAdultCategoryName`) behind a PIN. The PIN
 * itself is never stored — only a SHA-256 hash of it (see
 * `parental-control-pin.util.ts`) — so Settings can be freely backed up,
 * exported, or inspected without leaking it.
 */
export interface ParentalControlSettings {
    enabled: boolean;
    /** SHA-256 hex digest of the PIN. Empty only if a user actively clears it. */
    pinHash: string;
    /**
     * Case/accent-insensitive substrings matched against category names to
     * auto-detect adult categories, both for the always-on default hiding
     * applied when a playlist's categories are first imported, and for the
     * "auto-hide adult categories" action in the category management
     * dialog. User-editable; starts from DEFAULT_ADULT_CATEGORY_KEYWORDS.
     */
    autoHideKeywords: string[];
}

/**
 * Starter keyword list for `isAdultCategoryName`. Deliberately kept to
 * short stems rather than every inflected form: providers name categories
 * inconsistently, but "adult" alone already matches "adults", "adulto",
 * "adultos", and "para adultos" as substrings, "porn" already matches
 * "porno", "pornografía"/"pornografia" and "pornographic", and "erotic"
 * already matches "erótico", "erótica", "eroticos" and "eroticas" once
 * accents are stripped (see `isAdultCategoryName`'s normalization). Adding
 * the inflected forms too would be redundant, not more thorough.
 */
export const DEFAULT_ADULT_CATEGORY_KEYWORDS: string[] = [
    'xxx',
    'adult',
    '+18',
    '18+',
...
/**
 * Default PIN is "6699" — a placeholder, not a secret. It exists so adult
 * categories are hidden immediately after install without forcing a setup
 * step first; the settings UI should make changing it easy to find and
 * should encourage doing so on first use.
 */
export const DEFAULT_PARENTAL_CONTROL_PIN = '6699';

/**
 * Precomputed `hashParentalControlPin(DEFAULT_PARENTAL_CONTROL_PIN)`.
 * Hardcoded (rather than hashed at startup) because hashing is async (Web
 * Crypto) and DEFAULT_SETTINGS/DEFAULT_PARENTAL_CONTROL_SETTINGS need to be
 * plain, synchronously-constructible objects. Covered by a unit test that
 * asserts it equals `await hashParentalControlPin('6699')`, so the two
 * cannot silently drift apart.
 */
export const DEFAULT_PARENTAL_CONTROL_PIN_HASH =
    '9d7c744cda80fe0ccd8fe720fb41ca19092b3842a93e0c349d2da97ee922f282';

export const DEFAULT_PARENTAL_CONTROL_SETTINGS: ParentalControlSettings = {
    enabled: true,
    pinHash: DEFAULT_PARENTAL_CONTROL_PIN_HASH,
    autoHideKeywords: [...DEFAULT_ADULT_CATEGORY_KEYWORDS],
};
